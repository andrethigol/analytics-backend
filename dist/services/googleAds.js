...
exports.getGoogleAdsMetrics = getGoogleAdsMetrics;
exports.getGoogleAdsChartData = getGoogleAdsChartData;
...
async function getGoogleAdsMetrics(refreshToken) {
  ...
  `);
    const row = result[0];
    return {
        spend: Number(((row?.metrics?.cost_micros || 0) / 1000000).toFixed(2)),
        impressions: Number(row?.metrics?.impressions || 0),
        clicks: Number(row?.metrics?.clicks || 0),
        ctr: Number(((row?.metrics?.ctr || 0) * 100).toFixed(2)),
        cpc: Number(((row?.metrics?.average_cpc || 0) / 1000000).toFixed(2)),
        conversions: Number((row?.metrics?.conversions || 0).toFixed(0)),
    };
}
// Dados por dia para o gráfico
async function getGoogleAdsChartData(refreshToken) {
    const client = getClient();
    const customer = client.Customer({
        customer_id: getCustomerId(),
        refresh_token: refreshToken,
...
// Top campanhas
async function getGoogleAdsCampaigns(refreshToken) {
    const client = getClient();
    const customer = client.Customer({
        customer_id: getCustomerId(),
        refresh_token: refreshToken,
